...
interface Props {
  products: ExoticProduct[]
}

export function ExoticProductGrid({ products }: Props) {
  return (
    <section className="bg-background px-4 py-20 sm:px-6 lg:px-8" aria-labelledby="exotic-products-heading">
      <div className="mx-auto max-w-7xl">
        <div className="mb-14 max-w-2xl">
          <p className="text-overline text-[var(--gold)]">The Exotic Collection</p>
          <h2 id="exotic-products-heading" className="mt-4 font-serif text-4xl font-bold text-foreground sm:text-5xl">
            Marble with a rare point of view
          </h2>
          <p className="mt-5 text-base leading-relaxed text-foreground/65">
            Explore distinctive natural stone selected for expressive interiors, sculptural details, and spaces that deserve something uncommon.
          </p>
        </div>

        {products.length === 0 ? (
          <div className="border border-border/60 px-6 py-16 text-center">
            <p className="font-serif text-2xl text-foreground">The first Exotic arrivals are being curated.</p>
            <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground">
              Contact our specialists to discover upcoming exotic marble and stone selections.
            </p>
            <Link href="/contact" className="mt-7 inline-flex items-center gap-2 border border-[var(--gold)] px-5 py-3 text-xs uppercase tracking-[0.16em] text-[var(--gold)] transition-colors hover:bg-[var(--gold)] hover:text-background">

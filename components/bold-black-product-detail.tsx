...
interface Props {
  product: BoldBlackProduct
}

export function BoldBlackProductDetail({ product }: Props) {
    ...
    <main className="min-h-screen bg-background">
      <section className="px-4 pb-20 pt-36 sm:px-6 lg:px-8 lg:pt-44">
        <div className="mx-auto max-w-7xl">
          <Link href="/products/collections/bold-black" className="mb-10 inline-flex items-center gap-2 text-xs uppercase tracking-[0.18em] text-muted-foreground transition-colors hover:text-[var(--gold)]">
          ...
          <div className="grid items-start gap-12 lg:grid-cols-[1.15fr_0.85fr] lg:gap-20">
            <div className="relative aspect-[4/3] overflow-hidden bg-muted">
              <Image src={product.landscapeImage} alt={`${product.name} marble slab in the Bold Black collection`} fill priority className="object-cover" sizes="(max-width: 1024px) 100vw, 60vw" />
            </div>

            <div className="lg:pt-8">
              <p className="text-overline text-[var(--gold)]">Bold Black Collection</p>
